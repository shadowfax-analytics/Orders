...
date(order_time) as 'Order_date2',
case a.source
when 0 then 'Seller'
...
when 8 then 'Pickup Drop App As Is' end as App_Source,
...
d.rider_id as Rider_id,concat(rp.first_name,' ',rp.last_name) as Rider_name,d.version as Version,
...
join coreengine_riderprofile rp on d.rider_id = rp.id
where date(a.order_time) between '2016-01-01' and '2016-01-08' and c.cluster_name not like "%test%" 
and a.status != 302 order by a.seller_id,Order_time